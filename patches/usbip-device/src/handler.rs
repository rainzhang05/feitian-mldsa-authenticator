...
impl UsbIpBusInner {
    ...
    pub fn try_send_pending(&mut self, ep_addr: usize) {
        ...
        let bytes_requested = cmd.transfer_buffer_length as usize;
        ...
        while let Some(mut data) = ep_in.data.pop_front() {
            let remaining = bytes_requested.saturating_sub(out_buf.len());
            let chunk_len = data.len();

            if remaining == 0 {
                // We've already satisfied the host request; push the current
                // data back so it can be used to service the next transfer.
                ep_in.data.push_front(data);
                break;
            }
            ...
            if chunk_len <= remaining {
                ...
                out_buf.extend_from_slice(&data[..remaining]);
                let leftover = data.split_off(remaining);
                ep_in.data.push_front(leftover);
            }

            // Control transactions are terminated by a short packet. When we
            // see one, the transfer is complete even if the host requested
            // more bytes than were queued by the device.
            if chunk_len < ep_in.max_packet_size as usize {
                break;
            }
        }
